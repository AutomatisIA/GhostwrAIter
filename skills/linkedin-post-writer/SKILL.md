...
**Les exemples de registre ne vivent PAS dans ce fichier.** Un exemple positif
est le levier le plus direct sur le registre, et il n a de valeur que s il vient
de la personne qui publiera le post. Or ce depot est public et part dans chaque
build : un post ecrit ici s appliquerait a tous les utilisateurs, qui ecriraient
alors avec la voix d un autre.

Ils appartiennent a l espace de travail de chaque utilisateur. Voir
`### Exemples de reference` plus bas.
...
Aucun exemple ici, et il ne doit jamais y en avoir dans ce fichier.

Ce depot est public et son contenu part dans chaque build distribue : un post
colle ici serait lu par tous les utilisateurs, et chacun ecrirait avec la voix
de la personne qui l a colle. « L auteur » de ce prompt n est pas celui qui
maintient le depot, c est celui qui utilise l application.

Les exemples de registre appartiennent donc a l espace de travail de chaque
utilisateur, pas au depot. Voir la note du meme nom dans
`docs/exploitation.md` pour l emplacement attendu.
